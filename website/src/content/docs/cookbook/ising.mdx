...
The evolution function ties it together: it computes the step size `tau` from
the total time `t` and the number of steps `n_steps`, then repeats the Trotter
step that many times. The `main` function calls `ising_evolve(4, 1.0, 1.0, 0.0,
3)` — 4 qubits, coupling J = 1.0, field h = 1.0, time t = 0.0, and 3 Trotter
