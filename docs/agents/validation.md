...
| `just test-ci` | Local CI parity: `ci-rust` + `ci-tooling` + `ci-docs-assert` + `ci-samples` (not the website build) |
| `just ci-rust` | What the `ci.yml` `rust` job runs (sets `QUON_REQUIRE_LIT`) |
| `just ci-samples` | Sample corpus catalog lint (schema, paths, category coverage, README sections, `ci: smoke` typecheck) — also run by the `ci.yml` `rust` job, right after `ci-rust` (ADR-0025 / #185) |
...
| -------- | ------- | --------- |
| [ci.yml](../../.github/workflows/ci.yml) `rust` | every push and PR | `just ci-rust`: fmt, clippy, release build (+ examples for lit oracles), `cargo test --workspace --exclude flux_verify` with `QUON_REQUIRE_LIT` so [`quonc/tests/lit.rs`](../../quonc/tests/lit.rs) hard-fails without lit/FileCheck/oracles, then Qiskit Aer: `test/verify/{bell,teleport,bernstein_vazirani,routing,grover,qft,ising,qaoa,shor}.py` with `QUONC=target/release/quonc`, then QEC Python smokes (`test_qec_stim_smoke`, `test_quon_qec_sinter`, `test_quon_qec_benchmarks` / #254); then `just ci-samples`: [`quonc/tests/samples_catalog.rs`](../../quonc/tests/samples_catalog.rs) lints `samples/catalog.yaml` and typechecks every `ci: smoke` entry (ADR-0025 / #185). |
