...
**quantum.dynamic**: The dynamic circuit MLIR dialect. Adds measurement (`!qubit → !bit`), reset, feed-forward (`cf.cond_br` on `!bit`), and barrier ops. Unitary sub-circuits are embedded as `unitary_region` blocks containing only `quantum.circ` ops. `frontend::lower` emits `quantum.dynamic` IR directly from `run { }` blocks — circuit callees are inlined into `unitary_region` / `if` bodies, with no intermediate staging dialect (the former `monadic_staging` ops / `monadic_lowering` pass were collapsed in #213 / ADR-0037).
