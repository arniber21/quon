// Negative: Use then Reset same cycle is order-independent (#256).
//
// RUN: not %quonc %s --verify-na 2>&1 | FileCheck %s
// CHECK: quantum.na verification failed
// CHECK: reset and entangled or moved in the same cycle
// CHECK: reset ordering
...
module {
  "quantum.na.schedule"() ({
    "quantum.na.layer"() ({
      ...
      %b = "quantum.na.reset"(%a) {atom = 0 : i64, duration_us = 10 : i64} : (!quantum.na.atom) -> !quantum.na.atom
    }) {cycle = 0 : i64} : () -> ()
