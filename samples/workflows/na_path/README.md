...
(`neutral-atom/repetition-d3-memory`) for that story. Catalog id here:
`workflows/na-path`, `ci: none` (deliberately not a second `ci: smoke`
compile of a program #185's review already flagged for CI dedup — see
`samples/catalog.yaml`'s `neutral-atom/repetition-d3-memory` row, which is
also `ci: none`).
...
$QUONC $SRC --target $TARGET --emit-resource-report report.md
```
